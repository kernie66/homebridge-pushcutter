...
class PcPlatform extends homebridgeLib.Platform {
  constructor (log, configJson, homebridge) {
    ...
    try {
      ...
      emitter.on('notifications', () => {
        this.log('Got notifications, set up %s', this.config.notifications)
        for (const i in this.config.notifications) {
          ...
          if (config.useWebhook) {
            ...
            if (!included) {
              ...
              this.log('%s includes %s = %s', this.notificationIds, config.notificationName, included)
            }
  ...
  async getNotifications (url, options) {
    try {
      ...
      const notifications = await got.get(url, options)
