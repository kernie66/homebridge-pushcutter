...
class PcPlatform extends homebridgeLib.Platform {
  constructor (log, configJson, homebridge) {
    ...
    try {
      ...
      if (this.config.useCustomApi) {
        let url = parseUrl(this.config.customUrl, true).href
        if (isUrlHttp(url)) {
          if (this.config.customUrl != url) {
            this.log('Custom URL automatically adjusted from "%s" to "%s"')
          } else {
          ...
          this.warn('The custom URL "%s" is not a valid URL', this.config.customUrl)
