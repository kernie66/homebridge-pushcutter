  ...
  constructor (log, configJson, homebridge) {
    ...
    try {
      ...
      if (this.config.useApiKey) {
        let url = parseUrl(this.config.apiUrl, true)
        if (isUrlHttp(url.href)) {
          if (this.config.apiUrl != url) {
            this.warn('API URL automatically adjusted, please correct config file')
          }
          this.log('API URL is: %s', url.href)
          this.config.apiUrl = url.href
