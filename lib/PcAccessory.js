...
class PcAccessory extends homebridgeLib.AccessoryDelegate {
  constructor(platform, params) {
    ...
    if (this.useWebhook) {
      ...
      let path;
      try {
        let fullUrl = new URL(this.webhook);
        host = fullUrl.host;
        path = fullUrl.pathname;
        this.log(
          'Webhook split into host %s and path %s',
          host,
          path
        );
      } catch (error) {
        this.console.error();
        'Error in webhook URL %s: %s', this.webhook, error.message;
      }
      ...
      this.pcClient
        .on('error', (error) => {
          this.log(
            'Webhook request %d: Error = %s %s',
            error.request.id,
            error.request.method,
            error.request.resource
          );
          this.warn(
            'Webhook request %d: Error = %d, %s',
            error.request.id,
            error.statusCode,
            error.statusMessage
          );
        ...
        .on('request', (request) => {
          this.debug(
            'Webhook request %d: Request = %s, Resource = %s',
            request.id,
            request.method,
            request.resource
          );
          this.vdebug(
            'Webhook request %d: Request = %s, URL = %s',
            request.id,
            request.method,
            request.url
          );
        ...
        .on('response', (response) => {
          this.vdebug(
            'Webhook request %d: Response = %j',
            response.request.id,
            response.body
          );
          this.debug(
            'Webhook request %d: Response = %d %s',
            response.request.id,
            response.statusCode,
