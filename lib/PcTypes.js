...
class PcTypes extends homebridgeLib.CustomHomeKitTypes {
  constructor(homebridge) {
    ...
    this.createCharacteristicClass('Title', uuid('0A6'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY],
    }, 'Pushcut title (current)')

    this.createCharacteristicClass('Text', uuid('0A7'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY],
    }, 'Pushcut text (current)')

    this.createCharacteristicClass('NewTitle', uuid('0B6'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY, this.Perms.WRITE],
    }, 'Pushcut title (enter new)')

    this.createCharacteristicClass('NewText', uuid('0B7'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY, this.Perms.WRITE],
    ...
    this.createCharacteristicClass('Sound', uuid('0AA'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY],
    }, 'Notification sound (current)')

    this.createCharacteristicClass('SetSound', uuid('0AB'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY, this.Perms.WRITE],
    }, 'Notification sound (set new)')

    this.createCharacteristicClass('Devices', uuid('0AC'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY],
    }, 'Notificattion devices (current)')

    this.createCharacteristicClass('SetDevices', uuid('0AD'), {
      format: this.Formats.STRING,
      perms: [this.Perms.READ, this.Perms.NOTIFY, this.Perms.WRITE],
