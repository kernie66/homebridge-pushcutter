...
//    this.url = switchAccessory.url
  ...
  async setOn () {
    ...
    if (!this.switchOn) {
      ...
      if (this.pcTitle) {
        let pcTitle = replace(this.pcTitle, textObject, delimiter)
        this.log('%o, %s', textObject, pcTitle)
        jsonObject = {'title': pcTitle}
        json = Object.assign(json, jsonObject)
      } 
      if (this.pcText) {
        let pcText = replace(this.pcText, textObject, delimiter)
        this.log('%o, %s', textObject, pcText)
        jsonObject = {'text': pcText}
        json = Object.assign(json, jsonObject)
      }
      this.log('Client: %o, Notification: %s, JSON: %s', this.pcClient, this.notification, json)
