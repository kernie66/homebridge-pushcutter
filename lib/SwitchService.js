...
class SwitchService extends homebridgeLib.ServiceDelegate {
  constructor (switchAccessory, params = {}) {
    ...
    this.notificationName = switchAccessory.notificationName
    this.useWebhook = switchAccessory.useWebhook
    this.webhook = switchAccessory.webhook
    ...
    // this.url = this.api.url + this.notificationName
...
//    if (this.json) {
      this.log('Service - Url: %s, Options: %s, JSON: %s', this.url, this.options, this.options.json)
//    }
...
//      this.setOn(switchAccessory)
  ...
  async setOn () {
    ...
    if (!this.switchOn) {
      ...
      this.log('Url: %s, Options: %s, JSON: %s', this.url, this.options, this.options.json)

      got.post(this.url, this.options)
      .then ( res => {
        json = res.body
